...
```
MoneyIQ Application
├── Frontend (Next.js 16.1.6) [money-iq repository]
│   ├── Dashboard Components (Modular Architecture)
│   ├── Authentication System
│   ├── Real-time Data Display
│   └── WebSocket Client Integration
│
├── Separate WebSocket Server [money-iq-ws repository]
│   ├── Angel One SmartAPI Integration (stock-server.js)
│   ├── Redis Caching Layer
...
- WebSocket server runs in separate repository (money-iq-ws) on port 8080
...
- Decided on completely separate WebSocket server in separate repository
...
WebSocket Client (useWebSocket.ts)
    ↓ (WebSocket connection to money-iq-ws:8080)
Stock Server (stock-server.js in money-iq-ws)
...
```json
{
	...
	"dev-ws": "cd ../money-iq-ws && npm run dev", // WebSocket server (dev mode)
	"start-ws": "cd ../money-iq-ws && npm run start", // WebSocket server (prod mode)
